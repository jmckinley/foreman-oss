...
Supervisor for multiple concurrent Codex projects. Schedules recurring cadences,
...
- All timestamps stored as ISO 8601 UTC strings. Cron expressions are interpreted in local
  time by Codex, so convert at write time and record both.
- `run_id` is a ULID.
...
- Memory precedence between `AGENTS.md` and `Codex.local.md` is documented inconsistently.
